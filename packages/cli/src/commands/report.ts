...
export async function reportCommand(options: ReportOptions) {
  console.log(chalk.bold.cyan('\n📊 Generating DSS Certification Report\n'));
...
**Date:** ${date}  
**Generated by:** @vaultbricks/dss-cli v1.2.0
...
`;
  ...
  for (const cat of categories) {
    ...
    report += `**Status:** ${cat.status === 'pass' ? '✅ PASS' : cat.status === 'warn' ? '⚠️ WARNING' : '❌ FAIL'}\n\n`;
  ...
  if (percentage >= targetThreshold) {
    report += `✅ **Certification Status:** This project **MEETS** the requirements for ${options.level.toUpperCase()} certification.\n\n`;
  } else {
    report += `❌ **Certification Status:** This project **DOES NOT YET MEET** the requirements for ${options.level.toUpperCase()} certification (requires ${targetThreshold}%).\n\n`;
  }
  ...
  report += `\n---\n\n`;
  report += `*Generated by @vaultbricks/dss-cli - https://github.com/VaultBricks/DSS*\n`;
  ...
  if (['silver', 'gold'].includes(level)) {
    ...
    results.push({
      ...
      recommendations: !hasFuzzing 
        ? ['Add fast-check fuzzing tests', 'Use @vaultbricks/dss-test helpers'] 
        : undefined
    });
  }
